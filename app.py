...
# =================================================================
# 2. FUNGSI CORE (LOGIKA VERIFIKASI FISIK)
# =================================================================
...
def get_master_data():
    try:
        ...
        # JAMINAN: Kolom KETERANGAN di Master harus selalu kosong
...
def get_existing_result(toko_code, version):
    """Fungsi Inti: Verifikasi fisik file di Cloudinary (Bypass Cache)"""
    try:
        p_id = f"pareto_nkl/hasil/Hasil_{toko_code}_v{version}.xlsx"
        # Gunakan timestamp unik agar Cloudinary CDN memberikan data real-time
        url = f"https://res.cloudinary.com/{st.secrets['cloud_name']}/raw/upload/v1/{p_id}?t={int(time.time())}"
        
        # PING: Cek apakah file benar-benar ada di server
        ...
        if resp.status_code == 200:
            ...
            # Jika status bukan 200 (misal 404), artinya file sudah dihapus
            return None
    except: return None
...
if st.session_state.page == "HOME":
    ...
    if df_m_prog is not None:
        ...
        with col_a:
            st.write("### 📊 Progres Toko Input PER AM")
        ...
        with col_b:
            st.write("### 📊 Progres Toko Input PER AS")
    ...
    with tab_login:
        ...
        if st.button("LOG IN", type="primary", use_container_width=True):
            st.cache_data.clear() # Clear cache saat login
            try:
    ...
    with tab_daftar:
        ...
        if st.button("DAFTAR", use_container_width=True):
            if d_nik and d_pw == d_cpw:
                db = requests.get(f"https://res.cloudinary.com/{st.secrets['cloud_name']}/raw/upload/v1/{USER_DB}?t={int(time.time())}").json()
                if d_nik in db: st.warning("NIK sudah ada.")
                else:
                    db[d_nik] = d_pw
                    if update_user_db(db): st.success("Berhasil!")
    ...
    if st.button("Masuk Admin"):
        if pw == "icnkl034": st.cache_data.clear(); st.session_state.page = "ADMIN_PANEL"; st.rerun()
        else: st.error("Salah!")
    if st.button("Kembali"): st.session_state.page = "HOME"; st.rerun()

# =================================================================
# 4. ADMIN PANEL (LOGIKA FIX GABUNGAN)
# =================================================================
    ...
    with tab_mas:
        ...
        if f_up and st.button("🚀 Update Master"):
            ...
            st.cache_data.clear()
            st.success("Berhasil!"); time.sleep(3); st.rerun()

        st.divider()
        st.subheader("🗑️ Hapus Master Aktif")
        opsi_hapus_hasil = st.checkbox("Ikut hapus seluruh hasil input user?", value=False)
        if st.button("🔥 Hapus Master", type="primary"):
            ...
            st.cache_data.clear(); st.success("Terhapus!"); time.sleep(3); st.rerun()

    with tab_usr:
        st.subheader("Reset Password")
        nik_m = st.text_input("Ketik NIK:")
        db_u = requests.get(f"https://res.cloudinary.com/{st.secrets['cloud_name']}/raw/upload/v1/{USER_DB}?t={int(time.time())}").json()
        if nik_m in db_u:
            p_baru = st.text_input("Password Baru:", type="password")
            if st.button("Update"):
                db_u[nik_m] = p_baru
                if update_user_db(db_u): st.success("Sukses!"); time.sleep(3); st.rerun()
    ...
    with tab_res:
        if st.button("🔥 RESET HASIL INPUT", type="primary"):
            ...
            if pids: cloudinary.api.delete_resources(pids, resource_type="raw")
            st.cache_data.clear(); st.success("Bersih!"); time.sleep(3); st.rerun()

    if st.button("🚪 Logout Admin", use_container_width=True): st.cache_data.clear(); st.session_state.page = "HOME"; st.rerun()

# =================================================================
# 5. USER INPUT (LOGIKA MATCHING REAL-TIME)
# =================================================================
    ...
    if df_m is not None:
        ...
        c1, c2 = st.columns(2); c1.metric("KDTOKO:", val_kdtoko); c2.metric("AS:", val_as)

        # --- LOGIKA SINKRONISASI REAL-TIME ---
        # 1. Selalu mulai dari struktur Master terbaru
        data_toko_final = df_sel.copy()
        data_toko_final['PLU'] = data_toko_final['PLU'].astype(str).str.strip()

        # 2. Cek apakah file fisik hasil benar-benar ada di Cloud
        existing_df = get_existing_result(val_kdtoko, v_master)
        
        if existing_df is not None:
            # Jika file ADA: Lakukan merge (tempel keterangan)
            ...
            st.success(f"✅ Isian lama untuk Seri {v_master} dimuat.")
        else:
            # Jika file TIDAK ADA: Pastikan kolom keterangan kosong total
            st.info("ℹ️ Data baru / hasil input sebelumnya telah di-reset.")
            data_toko_final['KETERANGAN'] = ""

        # Cleaning data untuk editor
        data_toko_final['KETERANGAN'] = data_toko_final['KETERANGAN'].fillna("")
        for col in ['PLU', 'DESC', 'KETERANGAN']: data_toko_final[col] = data_toko_final[col].astype(str).replace(['nan','NaN'], '')
        ...
        # Key unik berbasis konten data (Hash) agar editor reset jika Master/Cloud berubah
        data_hash = hashlib.md5(pd.util.hash_pandas_object(data_toko_final[disp_cols]).values).hexdigest()
        edited_df = st.data_editor(data_toko_final[disp_cols], column_config=config, hide_index=True, use_container_width=True, key=f"ed_{val_kdtoko}_{data_hash}")
        ...
        if st.button("🚀 Simpan Hasil Input", type="primary", use_container_width=True):
            if edited_df['KETERANGAN'].apply(lambda x: str(x).strip() == "").any():
                ...
                st.cache_data.clear(); st.success("✅ TERSIMPAN!"); time.sleep(1); st.rerun()

    if st.button("Logout"): st.cache_data.clear(); st.session_state.page = "HOME"; st.rerun()
