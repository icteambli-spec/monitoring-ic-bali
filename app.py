...
# =================================================================
# 2. FUNGSI CORE & FIX LOGIN (RETRY LOGIC)
# =================================================================

def clear_all_caches():
    st.cache_data.clear()
    if 'user_db_cache' in st.session_state:
        del st.session_state.user_db_cache

def get_user_db_safe():
    """Mencegah Database User Error dengan Retry & Cache Session"""
    if 'user_db_cache' in st.session_state:
...
# =================================================================
# 3. ROUTING & HOME (BERPATOKAN PADA SKRIP INTI)
# =================================================================
...
if st.session_state.page == "HOME":
    ...
    if not df_m_prog.empty:
        ...
        # Detail Expander (Kembali Muncul Sesuai Skrip Inti)
    ...
    with tab_daftar:
        ...
        if st.button("DAFTAR", use_container_width=True):
            if d_nik and d_pw == d_cpw:
                db = get_user_db_safe()
                if db and d_nik in db: st.warning("NIK sudah ada.")
                else:
                    db[d_nik] = d_pw
...
# =================================================================
# 4. ADMIN PANEL (BERPATOKAN PADA SKRIP INTI)
# =================================================================
    ...
    with tab_mas:
        ...
        opsi_h = st.checkbox("Ikut hapus hasil input?")
...
# =================================================================
    ...
    if not df_m.empty:
        ...
        df_sel_store = df_f_am[df_f_am['NAMA TOKO'] == sel_nama_toko]
        val_kdtoko, val_as = str(df_sel_store['KDTOKO'].iloc[0]), str(df_sel_store['AS'].iloc[0])
        st.columns(2)[0].metric("KDTOKO:", val_kdtoko); st.columns(2)[1].metric("AS:", val_as)
        ...
        edited_minus = st.data_editor(
            ...
            column_config={**config_view, "KETERANGAN": st.column_config.TextColumn("KETERANGAN (Wajib Isi)", required=True)}, 
            hide_index=True, use_container_width=True, key=f"edit_nk_{val_kdtoko}"
        )
        ...
        st.dataframe(
            ...
            column_config=config_view, hide_index=True, use_container_width=True
        )
        ...
        if st.button("🚀 Simpan Hasil Input", type="primary", use_container_width=True):
            if edited_minus['KETERANGAN'].apply(lambda x: str(x).strip() == "").any():
                ...
                clear_all_caches(); st.success("✅ TERSIMPAN!"); time.sleep(1); st.rerun()
