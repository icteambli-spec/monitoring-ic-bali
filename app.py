...
def get_master_data():
    try:
        ...
        # 2. Pembersihan data untuk mencegah APIException
        for col in df.columns:
            # Mengisi nilai kosong (NaN) dengan string kosong agar editor stabil
            df[col] = df[col].fillna("")
            
        # 3. Paksa PRDCD tetap string (mencegah format scientific 1.0E+7)
        if 'PRDCD' in df.columns:
            df['PRDCD'] = df['PRDCD'].astype(str)
            
        return df
    except Exception as e:
        st.error(f"Error load master: {e}")
        return None
...
if st.session_state.page == "LOGIN":
    ...
    if f and st.button("Publish Master"):
        ...
        st.success("Berhasil Publish!"); st.cache_data.clear()
    if st.button("Logout"): st.session_state.page = "LOGIN"; st.rerun()

# =================================================================
# 4. USER INPUT (FIXED SYNTAX & LOGIC)
# =================================================================
    ...
    if df_m is not None:
        ...
        if not data_toko.empty:
            ...
            config = {
                ...
                "RP JUAL": st.column_config.TextColumn("RP JUAL", disabled=True), 
                "PENJELASAN": st.column_config.TextColumn("PENJELASAN", required=True)
            }

            # Editor dengan key unik berbasis toko dan jumlah baris
            ...
            st.warning("Data tidak tersedia untuk toko ini.")
    else:
